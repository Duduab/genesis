...
export default function AILoadingIndicator() {
  ...
  return (
    <div
      className="animate-slide-up-fade flex items-end gap-2.5"
      role="status"
      aria-live="polite"
      aria-busy="true"
    ...
    >
      ...
      />
      <div className="flex w-full max-w-[85%] flex-col gap-2.5 rounded-2xl rounded-es-sm bg-white px-4 py-3 shadow-sm ring-1 ring-surface-200/60 sm:min-w-[220px]">
        <div className="flex items-center gap-3">
          ...
          <span className="min-w-0 text-xs font-medium text-surface-400">{t('chat.aiTyping')}</span>
        </div>

        <div className="orchestrator-thinking-track" aria-hidden>
          <div className="orchestrator-thinking-fill" />
        </div>
      </div>
    </div>
  )
